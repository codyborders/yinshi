...
@pytest.mark.asyncio
async def test_manager_enqueues_restore_retry_for_failed_deletion_archive() -> None:
    """A deleting archive left by one failed deletion should stay restorable."""
    from datetime import datetime, timezone
    ...
    manager = ManagedBackupManager(
        ...
        start_restore=start_restore,
        now=lambda: datetime(2026, 8, 12, 12, 0, tzinfo=timezone.utc),
        new_id=lambda: "job-restore-retry",
    )

    job = manager.enqueue_restore("user-1", "archive-1")

    assert job.job_id == "job-restore-retry"
...
async def test_manager_restore_executes_download_guest_restore_and_activation(
    tmp_path, cleanup_pending: bool
) -> None:
    ...
    class Provider:
        ...
        async def read_file(self, name: str, **_values) -> bytes:
            ...
            return (
                '{"cleanup_pending":'
                f"{'true' if cleanup_pending else 'false'}"
                ',"job_id":"job-restore","status":"restored"}'
            ).encode("ascii")
