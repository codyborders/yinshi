"""Tests for runtime pi version and upstream release-note endpoints.
...
def test_pi_release_notes_route_returns_runtime_and_releases(
    ...
    """The endpoint should merge runtime, deployment, and release data."""
    import yinshi.services.pi_releases as pi_releases
    ...
    async def fake_fetch_releases(repository: str) -> dict[str, Any]:
        assert repository == "earendil-works/pi"
        release_prefix = "https://github.com/earendil-works/pi"
        release_url = f"{release_prefix}/releases/tag/v0.80.6"
    ...
    body = response.json()
    assert body["package_name"] == "@earendil-works/pi-coding-agent"
    assert body["installed_version"] == "0.80.6"
    assert body["latest_version"] == "0.80.6"
    assert body["node_version"] == "v22.19.0"
    assert body["update_policy"] == "Updated through reviewed lockfile deployments"
    assert "update_status" not in body
...
async def test_get_pi_release_notes_reports_fetch_failures(
    ...
    assert payload["installed_version"] == "0.80.6"
