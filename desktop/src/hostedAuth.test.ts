...
import {
  HOSTED_AUTH_CLOCK_SKEW_SECONDS,
...
it("accepts full-duration tokens when the desktop clock runs behind the server", async () => {
  const serverNow = 1_700_000_000;
  const clientNow = serverNow - 30;
  const lease = createLease(serverNow, serverNow + 30 * 24 * 60 * 60);
  const tokenResponse = () =>
    new Response(
      JSON.stringify({
        token_type: "Bearer",
        access_token: "memory-only-access-token",
        access_token_expires_at: serverNow + 15 * 60,
        refresh_token: "keychain-refresh-token",
        refresh_token_expires_at: serverNow + 90 * 24 * 60 * 60,
        account_lease: lease.token,
        account_lease_expires_at: serverNow + 30 * 24 * 60 * 60,
        device_id: "device-id",
        signing_public_key: lease.signingPublicKey,
        user: { id: "user-id", email: "user@example.com" },
      }),
  ...
  const session = await readHostedDesktopTokenResponse({
    ...
    currentTimeSeconds: clientNow,
  });
  expect(session.profile.refreshToken).toBe("keychain-refresh-token");
      ...
      response: new Response(
        JSON.stringify({
          ...
          access_token_expires_at:
            clientNow + 15 * 60 + HOSTED_AUTH_CLOCK_SKEW_SECONDS + 1,
          refresh_token: "keychain-refresh-token",
          refresh_token_expires_at: serverNow + 90 * 24 * 60 * 60,
          account_lease: lease.token,
          account_lease_expires_at: serverNow + 30 * 24 * 60 * 60,
  ...
  ).rejects.toThrow("hosted desktop authentication response is invalid");
});

it("accepts a ten-minute authorization window despite clock skew", async () => {
  const currentTimeSeconds = Date.parse("2023-11-14T22:08:20Z") / 1_000;
